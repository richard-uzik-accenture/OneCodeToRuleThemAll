...
### 1.1 Hand-rolled `dragging` state duplicates framer-motion's `whileDrag`
...
### 1.2 New `isDragging` ref duplicates the existing `preReorderTasks.current` signal
...
### 1.3 `useCallback` wrapping on `reorderTasks`/`commitReorder` buys no referential stability
...
- [src/hooks/useLongPressDrag.ts:22-24](../src/hooks/useLongPressDrag.ts#L22-L24): `cancel()` unconditionally blurs `document.activeElement`, which now also fires on every completed mouse-drag release, not just the touch long-press case the comment describes. Confirm this is intentional.
...
### 2.1 Give the duel commit its own module — **Strong**
...
Pick one of the three architecture candidates (2.1 recommended) to take through the grilling loop, and apply the three fixes from the code-quality review (1.1–1.3) to clear the approval bar on the current drag-and-drop diff.
